...
class MenuFilter {
  ...
  // 문장·설명·주의문에서 자주 나오는 표시(어미·조사 등)
  static final RegExp _sentence = RegExp(
      r'[。！？…♪、]|※|ございません|ありません|できます|ください|いたします|します|です|ます|など|または|より|まで|から|使用|注意|税込|税抜|付き|入り|選べる|お選び');

  // 카테고리 제목·비메뉴 단어 (일본어 원문 또는 한국어 번역이 정확히 일치하면 제외)
  static const Set<String> _headers = {
    // 일본어
    '酒の肴', '肴', '料理', '一品料理', '逸品料理', '一品', '逸品', 'お通し', 'おすすめ',
    '本日のおすすめ', '本日', 'メニュー', '串焼き', '博多串焼き', '豚串', '鶏串', '野菜',
    '巻き物', '揚げ物', '揚げもの', '焼き物', '焼きもの', '甘味', 'ご飯もの', 'ご飯物',
    'サラダ', 'ドリンク', 'ドリンクメニュー', '飲み物', 'お飲み物', 'アルコール',
    'デザート', 'セットメニュー', 'コース', 'サイドメニュー', 'サイド', 'フード',
    'ランチ', 'ディナー', '選べる小鉢の彩り', 'おまかせ', '国産米使用', '新名物',
    '博多', '名物', '当店', '限定', '数量限定', 'お食事', '食事', 'お通し代',
    // 한국어 번역
    '술안주', '안주', '요리', '일품요리', '일품', '추천', '오늘의 추천', '메뉴', '꼬치구이',
    '야채', '말이', '튀김류', '디저트', '샐러드', '음료', '드링크', '알코올', '코스',
    '사이드', '푸드', '신메뉴', '하카타', '명물', '한정', '수량한정', '식사',
  };
  ...
  static bool looksOrderable(String ja, String ko) {
    ...
    if (ja.length > 16) return false; // 너무 긴 줄 = 설명/문장
    if (_sentence.hasMatch(ja)) return false; // 문장·주의문·설명 어미
